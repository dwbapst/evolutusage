...
public class ForamVolumeMeter {
	...
	public double getForamVolume(int chambersCount, Genotype genotype, boolean isHaploid) {
		Double radius = isHaploid ?
				genotype.get(Genome.HAPLOID_FIRST_CHAMBER_RADIUS_NAME).getValue() :
				genotype.get(Genome.DIPLOID_FIRST_CHAMBER_RADIUS_NAME).getValue();
		Double growthFactor = genotype.get(Genome.GROWTH_FACTOR_NAME).getValue();
